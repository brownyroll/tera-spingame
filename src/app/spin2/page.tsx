...
import { showWinnerAlert, showErrorAlert } from "@/lib/swal";
...
export default function Spin2Page() {
  ...
  const handleItemSpinEnd = async (winningIndex: number) => {
    ...
    // Show item won alert with auto-close after 5 seconds
    const imageUrl =
      wonItem.image && wonItem.image.startsWith("data:image")
        ? wonItem.image
        : "/img/ani-merry.gif";
    ...
    Swal.fire({
      ...
      color: "#ffffff",
      didOpen: () => {
        // Auto start participant wheel after 5 seconds
        setTimeout(() => {
